...
INSERT INTO cliente VALUES (6,'Lasas S.A.','Antonio','Lasas','34916540145','34914851312','C/Leganes 15',NULL,'Fuenregionlabrada','Madrid','Spain','28945',8,154310);
...
INSERT INTO pedido VALUES (101,'2009-03-07','2009-03-27',NULL,'Rechazado','E	l pedido fue rechazado por el cliente',16);
...
select *from empleado where codigo_empleado=1;
...
select distinct  codigo_cliente from pago where codigo_cliente in (select codigo_cliente  from pedido  where  fecha_pedido between '2008-01-01' and '2008-12-31');
select distinct p.codigo_cliente from pago p  inner join pedido pe on p.codigo_cliente = pe.codigo_cliente where fecha_pedido between '2008-01-01' and '2008-12-31';
-- 9 Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos que no han sido entregados a tiempo.
select codigo_pedido,codigo_cliente,fecha_esperada,fecha_entrega from pedido  where fecha_entrega>fecha_esperada;
select codigo_pedido,codigo_cliente,fecha_esperada,fecha_entrega from pedido  where fecha_entrega>fecha_esperada;
-- 10. Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos cuya fecha de entrega ha sido al menos dos días antes de la fecha esperada.
select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega from pedido where fecha_entrega <= DATE_SUB(fecha_esperada, interval 2 day);
...
-- 1. Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representant de ventas.
select c.nombre_cliente , CONCAT(e.nombre, ' ', e.apellido1) as nombre_representante from cliente c inner join empleado e on c.codigo_empleado_rep_ventas= e.codigo_empleado;
-- 2. Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de su representantes de ventas.
select c.nombre_cliente , CONCAT(e.nombre, ' ', e.apellido1) as nombre_representante from cliente c inner join empleado e on c.codigo_empleado_rep_ventas= e.codigo_empleado INNER JOIN pago p ON c.codigo_cliente = p.codigo_cliente;
-- 3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.
select c.nombre_cliente , CONCAT(e.nombre, ' ', e.apellido1) as nombre_representante from cliente c inner join empleado e on c.codigo_empleado_rep_ventas= e.codigo_empleado where c.codigo_cliente not in (  SELECT codigo_cliente FROM pago ) ;
-- 4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
select distinct c.nombre_cliente , CONCAT(e.nombre, ' ', e.apellido1) as nombre_representante, ci.ciudad from cliente c inner join empleado e on c.codigo_empleado_rep_ventas= e.codigo_empleado INNER JOIN pago p ON c.codigo_cliente = p.codigo_cliente inner join oficina ci on e.codigo_oficina= ci.codigo_oficina;
-- 6. Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
select CONCAT(o.linea_direccion1, ' ',o.linea_direccion2) as direccion_oficina, c.ciudad from oficina o inner join empleado e on e.codigo_oficina= o.codigo_oficina inner join cliente  c on c.codigo_empleado_rep_ventas=e.codigo_empleado    where c.ciudad='Fuenlabrada'   ;
-- 7. Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
select c.nombre_cliente , CONCAT(e.nombre, ' ', e.apellido1) as nombre_representante, o.ciudad from cliente c inner join empleado e on c.codigo_empleado_rep_ventas= e.codigo_empleado inner join oficina o on o.codigo_oficina=e.codigo_oficina; 
-- 8. Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
select e.nombre as nombre_empleado ,j.nombre as nombre_jefe from empleado e inner join empleado j on e.codigo_jefe = j.codigo_empleado ;
-- 9. Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.
select distinct c.nombre_cliente from pedido p inner join cliente c  on c.codigo_cliente = p.codigo_cliente where p.fecha_entrega>p.fecha_esperada;
-- 10. Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
select c.nombre_cliente, p.gama  from cliente c inner join pedido pe on c.codigo_cliente = pe.codigo_cliente inner join detalle_pedido dp on  pe.codigo_pedido = dp.codigo_pedido inner join producto p on dp.codigo_producto = p.codigo_producto ;
select c.nombre_cliente, GROUP_CONCAT(DISTINCT p.gama SEPARATOR ', ') AS gamas_compradas from cliente c inner join pedido pe on c.codigo_cliente = pe.codigo_cliente inner join detalle_pedido dp on  pe.codigo_pedido = dp.codigo_pedido inner join producto p on dp.codigo_producto = p.codigo_producto GROUP BY c.codigo_cliente;
...
select  c.codigo_cliente, c.nombre_cliente from pago p  right join cliente c on  c.codigo_cliente = p.codigo_cliente where 	p.codigo_cliente is null;
...
select  c.codigo_cliente, c.nombre_cliente from pedido p  right join cliente c on  c.codigo_cliente = p.codigo_cliente where 	p.codigo_cliente is null;
-- 3. Devuelve un listado que muestre los clientes que no han realizado ningún pago y los que no han realizado ningún pedido.
select c.codigo_cliente, c.nombre_cliente from cliente c left join pago p on c.codigo_cliente = p.codigo_cliente left join pedido pe on  p.codigo_cliente= pe.codigo_cliente where  	p.codigo_cliente is null and p.codigo_cliente is null;
select c.codigo_cliente, c.nombre_cliente from pedido pe right join pago p on  p.codigo_cliente= pe.codigo_cliente right join cliente c on  c.codigo_cliente = p.codigo_cliente where  	p.codigo_cliente is null and p.codigo_cliente is null;
-- 4. Devuelve un listado que muestre solamente los empleados que no tienen una oficina asociada.
select e.codigo_empleado, e.nombre, e.apellido1, e.apellido2 from empleado e left join oficina o on e.codigo_oficina = o.codigo_oficina where o.codigo_oficina is null;
...
select CONCAT(e.nombre, ' ', e.apellido1) as nombre_representante from empleado e left join cliente c on c.codigo_empleado_rep_ventas=e.codigo_empleado where c.codigo_empleado_rep_ventas is  null;
...
select CONCAT(e.nombre, ' ', e.apellido1) as nombre_representante from empleado e left join cliente c on c.codigo_empleado_rep_ventas=e.codigo_empleado left join oficina o on e.codigo_oficina=o.codigo_oficina  where c.codigo_empleado_rep_ventas is  null or o.codigo_oficina is null;
-- 7. Devuelve un listado de los productos que nunca han aparecido en un pedido.
select p.nombre from producto p left join detalle_pedido dp on p.codigo_producto=dp.codigo_producto where dp.codigo_pedido is null;
...
select * from oficina o left join empleado e on e.codigo_oficina= o.codigo_oficina where e.codigo_empleado is null and o.codigo_oficina NOT IN (SELECT DISTINCT e.codigo_oficina FROM empleado e inner join cliente c on e.codigo_empleado = c.codigo_empleado_rep_ventas inner join  pedido pe on pe.codigo_cliente=  c.codigo_cliente inner join detalle_pedido dp on dp.codigo_pedido = pe.codigo_pedido inner  join producto pr on pr.codigo_producto=dp.codigo_producto  where  pr.gama='frutales'); 	
-- 9. Devuelve un listado con los clientes que han realizado algún pedido, pero no han realizado ningún pago.
select distinct c.codigo_cliente, c.nombre_cliente from cliente c  left join pedido pe on pe.codigo_cliente = c.codigo_cliente left join pago p on  p.codigo_cliente is null;
-- 10. Devuelve un listado con los datos de los empleados que no tienen clientes asociados y el nombre de su jefe asociado.
select  ROW_NUMBER() OVER () AS numero_secuencia ,CONCAT(e1.nombre, ' ', e1.apellido1) as nombre_empleado   from empleado e1  left join cliente c on c.codigo_empleado_rep_ventas=e1.codigo_empleado where  c.codigo_empleado_rep_ventas is null ;
select  ROW_NUMBER() OVER () AS numero_secuencia ,CONCAT(e1.nombre, ' ', e1.apellido1) as nombre_empleado   , CONCAT(e2.nombre, ' ', e2.apellido1) as nombre_jefe from empleado e1  inner join empleado e2 on e1.codigo_jefe = e2.codigo_empleado left join cliente c on c.codigo_empleado_rep_ventas=e1.codigo_empleado where  c.codigo_empleado_rep_ventas is null ;
select  ROW_NUMBER() OVER () AS numero_secuencia ,CONCAT(e1.nombre, ' ', e1.apellido1) as nombre_empleado   , CONCAT(e2.nombre, ' ', e2.apellido1) as nombre_jefe from empleado e1   left join cliente c on c.codigo_empleado_rep_ventas=e1.codigo_empleado left join empleado e2 on e1.codigo_jefe = e2.codigo_empleado where  c.codigo_empleado_rep_ventas is null ;

-- Consultas resumen
-- 1. ¿Cuántos empleados hay en la compañía?
select  count(*) as total_empleados from  empleado;
select row_number() over () as total_empleados from empleado;
...
select  count(*) as total_clientes from  cliente where ciudad = 'madrid';
-- 8. ¿Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M?
select  count(*) as total_clientes from  cliente where ciudad like 'm%';
-- 9. Devuelve el nombre de los representantes de ventas y el número de clientes al que atiende cada uno.
select e.nombre, count(c.codigo_cliente) as cantidad_clientes from empleado e  left join cliente c on c.codigo_empleado_rep_ventas= e.codigo_empleado where e.puesto='representante ventas' GROUP BY e.codigo_empleado, e.nombre ;
-- 10. Calcula el número de clientes que no tiene asignado representante de ventas.
select count(*) from cliente where codigo_empleado_rep_ventas is null;
-- 11. Calcula la fecha del primer y último pago realizado por cada uno de los clientes. El listado deberá mostrar el nombre y los apellidos de cada cliente.
select concat(c.nombre_contacto,' ' , c.apellido_contacto) as nombre_cliente , (select min(p.fecha_pago) from pago p where c.codigo_cliente=p.codigo_cliente) as primer_pago , (select max(p.fecha_pago) from pago p where c.codigo_cliente=p.codigo_cliente) as ultimo_pago from cliente c  ;
-- 12. Calcula el número de productos diferentes que hay en cada uno de los pedidos.
select codigo_pedido, count(codigo_producto) as  cantidad_productos from detalle_pedido group by codigo_pedido;
select codigo_pedido, count(*) as  cantidad_productos from detalle_pedido group by codigo_pedido;
...
select   codigo_producto, sum(cantidad) as  cantidad_productos from  detalle_pedido dp group by codigo_producto order by cantidad_productos desc limit 20;
-- 15. La facturación que ha tenido la empresa en toda la historia, indicando la base imponible, el IVA y el total facturado. La base imponible se calcula sumando el coste del producto por elnúmero de unidades vendidas de la tabla detalle_pedido. El IVA es el 21 % de la base imponible, y el total la suma de los dos campos anteriores.
SELECT dp.codigo_producto,  pr.precio_venta AS precio_unidad,SUM(dp.cantidad) AS total_vendido, SUM(dp.cantidad * pr.precio_venta) AS base_imponible, SUM(dp.cantidad * pr.precio_venta) * 0.21 AS iva, SUM(dp.cantidad * pr.precio_venta) * 1.21 AS total_facturado FROM detalle_pedido dp JOIN producto pr ON dp.codigo_producto = pr.codigo_producto GROUP BY dp.codigo_producto, pr.precio_venta;
select codigo_producto, precio_unidad,SUM(cantidad) AS total_vendido,SUM(cantidad * precio_unidad) AS base_imponible,SUM(cantidad * precio_unidad) * 0.21 AS iva,SUM(cantidad * precio_unidad) * 1.21 AS total_facturado from detalle_pedido group by codigo_producto, precio_unidad;
-- 16. La misma información que en la pregunta anterior, pero agrupada por código de producto.
select codigo_producto, precio_unidad,SUM(cantidad) AS total_vendido,SUM(cantidad * precio_unidad) AS base_imponible,SUM(cantidad * precio_unidad) * 0.21 AS iva,SUM(cantidad * precio_unidad) * 1.21 AS total_facturado from detalle_pedido group by codigo_producto, precio_unidad;
-- 17. La misma información que en la pregunta anterior, pero agrupada por código de productofiltrada por los códigos que empiecen por OR.
select codigo_producto, precio_unidad,SUM(cantidad) AS total_vendido,SUM(cantidad * precio_unidad) AS base_imponible,SUM(cantidad * precio_unidad) * 0.21 AS iva,SUM(cantidad * precio_unidad) * 1.21 AS total_facturado from detalle_pedido where codigo_producto like'OR%' group by codigo_producto, precio_unidad;
-- 18. Lista las ventas totales de los productos que hayan facturado más de 3000 euros. Se mostrará el nombre, unidades vendidas, total facturado y total facturado con impuestos (21%IVA)
select p.nombre, dp.precio_unidad,SUM(dp.cantidad) AS total_vendido,SUM(dp.cantidad * dp.precio_unidad) AS base_imponible,SUM(dp.cantidad * dp.precio_unidad) * 0.21 AS iva,SUM(dp.cantidad * dp.precio_unidad) * 1.21 AS total_facturado from detalle_pedido dp inner join producto p on dp.codigo_producto=p.codigo_producto group by dp.codigo_producto, dp.precio_unidad HAVING SUM(dp.cantidad * dp.precio_unidad) * 1.21 > 3000;
...
-- 3. Devuelve el nombre del producto del que se han vendido más unidades. (Tenga en cuent  que tendrá que calcular cuál es el número total de unidades que se han vendido de cada producto a partir de los datos de la tabla detalle_pedido. Una vez que sepa cuál es el códigodel producto, puede obtener su nombre fácilmente.)
select  p.nombre  ,sum(dp.cantidad) as total from detalle_pedido dp join producto p on p.codigo_producto=dp.codigo_producto  group by p.nombre order by total desc limit 1 ;
SELECT p.nombre, SUM(dp.cantidad) AS total FROM detalle_pedido dp JOIN producto p ON p.codigo_producto = dp.codigo_producto GROUP BY p.nombre HAVING SUM(dp.cantidad) = (SELECT MAX(total_unidades_vendidas) FROM (SELECT SUM(cantidad) AS total_unidades_vendidas FROM detalle_pedido GROUP BY codigo_producto) AS unidades_vendidas);
-- 4. Los clientes cuyo límite de crédito sea mayor que los pagos que haya realizado. (Sin utilizar INNER JOIN).
select c.nombre_cliente from cliente c join pago p on p.codigo_cliente = c.codigo_cliente where c.limite_credito>p.total;
select nombre_cliente from cliente  where limite_credito  > (select ifnull(sum(total), 0) from pago where codigo_cliente = cliente.codigo_cliente );
-- 5. Devuelve el producto que más unidades tiene en stock.
select nombre , max(cantidad_en_stock) as max_stock from producto group by  nombre order by max_stock desc limit 1;
select p.nombre, p.cantidad_en_stock AS max_unidades_en_stock from producto p where p.cantidad_en_stock = (select MAX(cantidad_en_stock) from producto);
--  Devuelve el producto que menos unidades tiene en stock.
select nombre , min(cantidad_en_stock) as min_stock from producto group by  nombre order by min_stock asc limit 1;
select p.nombre, p.cantidad_en_stock AS min_unidades_en_stock from producto p where p.cantidad_en_stock = (select min(cantidad_en_stock) from producto);
-- 7. Devuelve el nombre, los apellidos y el email de los empleados que están a cargo de Alberto Soria.
select concat(e.nombre,' ',e.apellido1) as nombre_empleado ,e.email from empleado e  where  e.codigo_jefe = (select codigo_empleado from empleado where  nombre='alberto');
select concat(e.nombre,' ',e.apellido1) as nombre_empleado ,e.email  from empleado e  join empleado e1 on e.codigo_empleado = e1.codigo_empleado where   e.codigo_jefe = (select codigo_empleado from empleado where  nombre='alberto');
...
select  concat(nombre, ' ', apellido1 ) as nombre_empleado, puesto from  empleado where codigo_empleado not in (select  codigo_empleado_rep_ventas from  cliente );
...
select concat(nombre,' ',apellido1,' ',apellido2) as nombre_empleado ,puesto,o.telefono from  empleado  join oficina o on empleado.codigo_oficina=o.codigo_oficina where  codigo_empleado not in (select codigo_empleado_rep_ventas from  cliente);


-- Subconsultas con EXISTS y NOT EXISTS
-- 1. Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago.
select c.nombre_cliente from cliente c where not exists(select 1 from pago  p where c.codigo_cliente=p.codigo_cliente);
-- 2. Devuelve un listado que muestre solamente los clientes que sí han realizado ningún pago.
select c.nombre_cliente from cliente c where exists(select 1 from pago  p where c.codigo_cliente=p.codigo_cliente);
-- 3. Devuelve un listado de los productos que nunca han aparecido en un pedido.
select p.nombre from  producto p where not exists ( select 1 from detalle_pedido dp where  p.codigo_producto= dp.codigo_producto);
-- 4. Devuelve un listado de los productos que han aparecido en un pedido alguna vez.
select p.nombre from  producto p where  exists ( select 1 from detalle_pedido dp where  p.codigo_producto= dp.codigo_producto);
