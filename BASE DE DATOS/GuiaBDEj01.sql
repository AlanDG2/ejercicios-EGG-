...
select sal_emp, comision_emp, (sal_emp+comision_emp) as Salario_total , nombre from empleados where  comision_emp >1000 order by Salario_total asc ;
select sal_emp, comision_emp, (sal_emp+comision_emp) as Salario_total , nombre from empleados where  comision_emp =0 order by Salario_total asc ;
select sal_emp, comision_emp,nombre  from empleados where sal_emp < comision_emp;
...
select id_depto , count(id_emp) as Empleados from empleados GROUP BY id_depto having count(id_emp)>3 ;
select id_depto , count(id_emp) as Empleados from empleados GROUP BY id_depto having count(id_emp)=0 ;
